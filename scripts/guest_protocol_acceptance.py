    ...
    image_policy = execute(
        ...
        request(
            ...
            [
                ...
                (
                    "import os;"
                    "walk=lambda root:[root]+[os.path.join(base,name) "
                    "for base,dirs,files in os.walk(root) for name in dirs+files];"
                    "writable_paths=['/var/lib/microvm/pnpm-store','/var/lib/microvm/pnpm-cache'];"
                    "template_path='/opt/microvm/next-template';"
                    "writable_entries=[path for root in writable_paths for path in walk(root)];"
                    "assert all((os.lstat(path).st_uid,os.lstat(path).st_gid)==(1000,1000) "
                    "for path in writable_entries);"
                    "assert all(os.access(root,os.W_OK) for root in writable_paths);"
                    "assert any(os.path.isfile(path) for path in writable_entries);"
                    "template_entries=walk(template_path);"
                    "assert all((os.lstat(path).st_uid,os.lstat(path).st_gid)==(0,0) "
                    "for path in template_entries);"
